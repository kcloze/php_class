...
class NewsArticlesController extends ManagerController
{
	...
	public function actionCreate()
	{
		...
		{   
			if( isset($_POST['NewsArticles']['recommand']) && is_array($_POST['NewsArticles']['recommand'])){
				$model->recommand=implode(',',$_POST['NewsArticles']['recommand']);
			}
			...
			if($model->save()){
			    ...
			    if($_POST['NewsArticles']['CateId']==85){
			    	...
			    	UserHelper::mzone_auto_bbs(2154920,9,3,$_POST['NewsArticles']['Title'],$content);
			    }
	...
	{
		$model=$this->loadModel($id);
		$model->recommand=explode(',', $model->recommand);
		//显示无限级分类
        $result=$this->getCategories();
		$category=$result['category'];
		$Tree=$result['Tree'];
