...
class MZBoss{
    ...
    public function __construct($mobile){
    	...
    	global $cfg_mzone;
    	if( 'off' == $cfg_mzone['boss_on'] )return true;
...
//		$datatrans = '13560471642~6000~1';
		...
		if (!$domHeard->loadXML($xml_heard)){
			$errors = libxml_get_errors();
			$filename = '/var/www/mzone/uploads/boss_error.txt';
			$somecontent = '';
			foreach ($errors as $error) {
				$somecontent .= $this->display_xml_error($error, $xml_heard);
			}
		...
		if (!$domBody->loadXML($xml_body)){
			$errors = libxml_get_errors();
			$filename = '/var/www/mzone/uploads/boss_error.txt';
			$somecontent = '';
			foreach ($errors as $error) {
				$somecontent .= $this->display_xml_error($error, $xml_body);
			}
			$handle = fopen($filename, 'a');
    		fwrite($handle, $somecontent);
    		fclose($handle);
