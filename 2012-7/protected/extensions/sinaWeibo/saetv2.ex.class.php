...
class SaeTClientV2
{
	...
	/**
	 * 获取两个用户之间的详细关注关系情况
	 * 对应API：{@link http://open.weibo.com/wiki/2/friendships/show}
	 * @param int $target_id 需要查询的用户uid
	 */
	function friends_show($uid){
		$params = array();
	...
	/**
	 * 关注一个用户
	 * 对应API：{@link http://open.weibo.com/wiki/2/friendships/create}
	 * @param int $uid 需要查询的用户uid
	 */
	function friends_create($uid){
		$params = array();
		$params['uid']=$uid;
	 ...
	 * 退出登录
	 */
    function account_end_session($token){
    	$params = array();
    	$params['access_token']=$token;
    	return $this->oauth->get( 'account/end_session', $params );
    }
	/**
	 * 获取隐私信息设置情况
	 *
